...
async def get_books(
    limit: int = Query(10, ge=1, le=100, description="Кількість записів (Limit)"),
    cursor: Optional[UUID] = Query(None, description="Курсор (ID останньої книги з попередньої сторінки)"),
    ...
    return await service.get_all(limit=limit, cursor=cursor, status=status_filter, author=author, sort_by=sort_by)
